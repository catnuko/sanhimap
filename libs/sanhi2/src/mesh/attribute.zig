...
pub const AttributeData = union(enum) {
    ...
    pub fn fromSlice(comptime T: type, slice: []const T, norm: bool) AttributeData {
        var list = std.ArrayList(T).initCapacity(lib.mem.getAllocator(), slice.len) catch unreachable;
        list.appendSliceAssumeCapacity(slice);
        return AttributeData.fromList(T, list,norm);
    }
