...
pub const Ellipsoid = struct {
    ...
    pub fn toCartesian(self: *const Self, cg: *const Cartographic) math.Vec3d {
        ...
        const sinc = stdmath.sin(c);
        const cosc = stdmath.cos(c);
        const sind = stdmath.sin(d);
        const cosd = stdmath.cos(d);

        const eSq = 2 * f - f * f; // 1st eccentricity squared ≡ (a²-b²)/a²
        const v = a / stdmath.sqrt(1 - eSq * sinc * sinc); // radius of curvature in prime vertical
    ...
    pub fn toCartographic(self: *const Self, vec: *const math.Vec3d) Cartographic {
        ...
        const p = stdmath.sqrt(x * x + y * y); // distance from minor axis
        const R = stdmath.sqrt(p * p + z * z); // polar radius

        // parametric latitude (Bowring eqn.17, replacing tanb = z·a / p·b)
        const tanb = (b * z) / (a * p) * (1 + ee2 * b / R);
        const sinb = tanb / stdmath.sqrt(1 + tanb * tanb);
        const cosb = sinb / tanb;

        // geodetic latitude (Bowring eqn.18: tang = z+ε²⋅b⋅sin³b / p−e²⋅cos³b)
        const g = if (stdmath.isNan(cosb)) 0 else stdmath.atan2(z + ee2 * b * sinb * sinb * sinb, p - e2 * a * cosb * cosb * cosb);

        // longitude
        const h = stdmath.atan2(y, x);

        // height above ellipsoid (Bowring eqn.7)
        const sing = stdmath.sin(g);
        const cosg = stdmath.cos(g);
        const v = a / stdmath.sqrt(1 - e2 * sing * sing); // length of the normal terminated by the minor axis
    ...
    pub fn intermediatePointTo(self: *const Self, p1: *const Cartographic, p2: *const Cartographic, fraction: f64) Cartographic {
        ...
        return if (stdmath.isNan(brng)) p1.clone() else self.destinationPoint(p1, dist * fraction, brng);
    }
    ...
    pub fn direct(self: *const Self, p: *const Cartographic, distance: f64, initialBearing: f64) DirectResult {
        ...
        const sina1 = stdmath.sin(a1);
        const cosa1 = stdmath.cos(a1);

        const tanU1 = (1 - f) * stdmath.tan(lat);
        const cosU1 = 1 / stdmath.sqrt((1 + tanU1 * tanU1));
        const sinU1 = tanU1 * cosU1;
        const o1 = stdmath.atan2(tanU1, cosa1); // o1 = angular distance on the sphere from the equator to P1
        ...
        const phi = stdmath.atan2(sinU1 * coso + cosU1 * sino * cosa1, (1 - f) * stdmath.sqrt(sina * sina + x * x));
        const l = stdmath.atan2(sino * sina1, cosU1 * coso - sinU1 * sino * cosa1);
        const C = f / 16 * cosSqa * (4 + f * (4 - 3 * cosSqa));
        const L = l - (1 - C) * f * sina * (o + C * sino * (cos2o + C * coso * (-1 + 2 * cos2o * cos2o)));
        const l2 = lon + L;

        const a2 = stdmath.atan2(sina, -x);
        return .{
    ...
    pub fn inverse(self: *const Self, p1: *const Cartographic, p2: *const Cartographic) ?InverseResult {
        ...
        const tanU1 = (1 - f) * stdmath.tan(lat1);
        const cosU1 = 1 / stdmath.sqrt((1 + tanU1 * tanU1));
        const sinU1 = tanU1 * cosU1;
        const tanU2 = (1 - f) * stdmath.tan(lat2);
        const cosU2 = 1 / stdmath.sqrt((1 + tanU2 * tanU2));
        ...
        while (math.abs(l - ll) > 1e-12 and iter < 1000) : (iter += 1) {
            sinl = stdmath.sin(l);
            cosl = stdmath.cos(l);
            sinSqo = stdmath.pow(f64, cosU2 * sinl, 2) + stdmath.pow(f64, cosU1 * sinU2 - sinU1 * cosU2 * cosl, 2);
            if (math.abs(sinSqo) < 1e-24) break; // co-incident/antipodal points (o < ≈0.006mm)
            sino = stdmath.sqrt(sinSqo);
            coso = sinU1 * sinU2 + cosU1 * cosU2 * cosl;
            o = stdmath.atan2(sino, coso);
        ...
        const a1 = if (math.abs(sinSqo) < math.eps_f64) 0 else stdmath.atan2(cosU2 * sinl, cosU1 * sinU2 - sinU1 * cosU2 * cosl);
        const a2 = if (math.abs(sinSqo) < math.eps_f64) math.pi else stdmath.atan2(cosU1 * sinl, -sinU1 * cosU2 + cosU1 * sinU2 * cosl);
