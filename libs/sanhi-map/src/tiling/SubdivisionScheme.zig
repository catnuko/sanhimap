...
const HalfQuadTreeSubdivisionScheme = struct {
    ...
    pub fn getLevelDimensionY(level: u32) u32 {
        return if (level != 0) math.pow(u32, 2, level - 1) else 1;
    }
    pub fn subdivisionSchemeI() SubdivisionScheme {
        return .{ .vtable = &.{
            .getSubdivisionX = getSubdivisionX,
...
test "SubdivisionScheme" {
    const testing = @import("std").testing;
    try testing.expectEqual(quadTreeSubdivisionScheme.getSubdivisionY(0), 2);
    try testing.expectEqual(halfQuadTreeSubdivisionScheme.getSubdivisionY(0), 1);
}
